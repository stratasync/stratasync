...
Returns the count of pending (unsynced) transactions and a boolean indicating whether there are any.
...
Returns a `sync` function to manually trigger a sync cycle, and an `isSyncing` flag.
...
function SyncStatusBar() {
  ...
  return (
    <div className="flex items-center gap-4 p-2 text-sm">
      ...
      {hasPending && <span className="text-yellow-600">{count} pending</span>}
