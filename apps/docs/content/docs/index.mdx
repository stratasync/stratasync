...
A local-first, server-sequenced sync engine for TypeScript, React, and Next.js. Reads come from a local store, so the UI stays fast. Writes enter an optimistic outbox, so mutations feel instant. A monotonic sync log on the server ensures all clients converge.

Based on the server-sequenced architecture [Linear](https://linear.app) described but never open-sourced. Extended with Yjs CRDT collaboration, undo/redo, and pluggable adapters.
