...
The `YjsDocumentManager` from `@stratasync/yjs` manages Yjs document instances and their connection to the sync server.
...
- [sync-yjs API Reference](/docs/packages/sync-yjs) -- Full API documentation for `YjsDocumentManager` and `YjsPresenceManager`.
