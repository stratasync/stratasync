...
Set up a local-first React app with real-time sync, offline support, and undo/redo.

Want to skip the manual setup? Scaffold a complete app with the Claude Code skill:
...
A model is a TypeScript class decorated with schema metadata. `@ClientModel` registers the model, and `@Property` marks each synced field.
...
The sync client orchestrates storage (where data lives locally), transport (how data reaches the server), and reactivity (how changes propagate to the UI).
...
`SyncProvider` manages the client lifecycle: it starts sync on mount and stops it on unmount.
...
Strata Sync provides hooks for reading single records and filtered lists. Both accept a generic type parameter for type-safe access.

### useModel -- single record

`useModel` fetches one model instance by ID. It integrates with React Suspense, so wrap the component in a `<Suspense>` boundary.
...
`useQuery` returns a reactive list of models matching a filter. It doesn't use Suspense; instead it returns `{ data, isLoading, error }`.
...
Use the sync client to create, update, and delete records. The client applies mutations optimistically and queues them for sync.
...
- [Architecture overview](/docs/architecture/overview) -- the layered architecture and package dependency graph
- [Sync protocol](/docs/architecture/sync-protocol) -- the server-sequenced sync model in depth
- [Data flow](/docs/architecture/data-flow) -- how mutations and deltas flow through the system
