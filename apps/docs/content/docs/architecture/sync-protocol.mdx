...
This protocol is based on the server-sequenced sync architecture Linear described but never open-sourced. Strata Sync is an independent implementation that adds Yjs CRDT integration, undo/redo, and pluggable adapters.
