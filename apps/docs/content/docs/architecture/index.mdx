...
The core is framework-agnostic. Adapters at the edges handle React, MobX, IndexedDB, and GraphQL. The sync protocol is based on [Linear's published design](/docs/architecture/sync-protocol#design-lineage).
...
Each layer builds on the one below. See the individual [package docs](/docs/packages) for API details.
...
React hooks and providers connect the sync client to the component tree. The Next.js package adds server-side bootstrap prefetching for fast first paint. See [React](/docs/packages/react) and [Next.js](/docs/packages/next) for the full hook API.
