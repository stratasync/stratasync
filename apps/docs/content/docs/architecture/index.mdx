...
Strata Sync has a layered architecture. The core is framework-agnostic; adapters at the edges handle React, MobX, IndexedDB, and GraphQL. This server-sequenced, client-optimistic model follows the architecture Linear published. See [Sync protocol](/docs/architecture/sync-protocol) for details.
