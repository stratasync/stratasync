...
Transactions are the atomic unit of mutation in Strata Sync. Every local change becomes a `Transaction` object that flows through the outbox, travels to the server via the transport layer, and gets confirmed by the sync protocol.

## Transaction interface

Every mutation in the system conforms to this structure.
...
All other factory functions delegate to this general-purpose factory.
...
Creates a `"D"` (delete) transaction. You must pass the original data so undo can re-create the model.
...
Creates the inverse transaction for a given transaction. Returns `null` if the transaction can't be undone (for example, a delete without original data).
...
You can serialize transactions to a compact format for storage (IndexedDB outbox) or transport (network payloads).
...
The server returns these result objects after you submit transactions.
