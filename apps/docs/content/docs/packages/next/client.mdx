...
These props configure the sync provider behavior.
...
The provider follows this sequence on mount.

1. Resolves the client (calls the factory if a function is provided).
...
The provider passes `autoStart={false}` and `autoStop={false}` to the inner `SyncProvider` because it manages the lifecycle itself.

### Lazy client factory

Pass a factory function to defer client creation until the component mounts.
...
This is useful when client configuration depends on values only available in the browser (cookies or localStorage).

## Bootstrap decoding

These utilities decode bootstrap snapshots serialized on the server. For server-side serialization functions, see [Server utilities](/docs/packages/sync-next/server).
...
Seeds the local storage adapter with data from a bootstrap snapshot. Pre-populates IndexedDB so the client can skip the network bootstrap.
...
The package re-exports most hooks from `@stratasync/react` with the `"use client"` directive. You can import them directly from `@stratasync/next` in your client components.
