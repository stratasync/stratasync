...
`@stratasync/y-doc` provides Yjs CRDT integration for Strata Sync. It manages Y.Doc instances for collaborative text editing, handles the Yjs sync protocol, and tracks presence for each collaborative document.
...
The package groups its exports into four categories.
...
The package is framework-agnostic -- it has no React or browser-specific dependencies beyond `yjs`.
