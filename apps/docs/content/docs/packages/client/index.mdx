...
The React hooks in `@stratasync/react` are thin wrappers around `SyncClient`. See [React](/docs/packages/react) for details.
...
See [Undo and redo](/docs/packages/client/undo-redo) for details.
...
See [Event system](/docs/packages/client/events) for the full reference.
