...
`@stratasync/transport-graphql` implements the `TransportAdapter` interface with NDJSON bootstrap streaming, REST and GraphQL mutations, WebSocket delta subscriptions, and Yjs collaborative editing -- all with built-in retry logic and exponential backoff.

## What it provides

This package includes the transport adapter, WebSocket management, and streaming utilities.
...
The transport calls the auth provider before each HTTP request and WebSocket subscription.
...
The `batchLoad` method works the same way but fetches specific model instances.
...
The adapter supports two mutation modes: REST (default) and GraphQL.
...
When you provide a `mutationBuilder`, the adapter sends mutations as a GraphQL `mutation` document. The builder receives each transaction and its batch index, and returns a `GraphQLMutationSpec`.
...
You can fetch deltas via REST for catch-up after reconnection. Use `fetchDeltas` for a single page or `fetchAllDeltas` for automatic pagination.
...
The adapter provides a `YjsTransportAdapter` for collaborative editing. It bridges Yjs protocol messages over the existing WebSocket connection that `WebSocketManager` manages, so you don't need a separate WebSocket.
...
`@stratasync/transport-graphql` connects the sync client to the server.
...
The sync client depends on a `TransportAdapter` implementation at runtime. The same interface can be implemented with other protocols (gRPC, custom WebSocket).
