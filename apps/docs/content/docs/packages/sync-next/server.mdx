...
The server utilities in `@stratasync/next/server` allow you to prefetch sync data on the server and pass it to client components. This reduces time-to-interactive by eliminating the client-side bootstrap network request.

Import server utilities from the `/server` sub-path:
...
export default async function DashboardPage() {
  // getServerToken() is your authentication helper that returns the user's JWT
...
The snapshot contains the complete bootstrap response: all model rows with their data, the sync cursor positions, and metadata for staleness checking.

## serializeBootstrapSnapshot

Serializes a bootstrap snapshot for transfer across the Server Component boundary. The function supports optional gzip compression.
...
When compression is available (Node.js 18+ / modern browsers), the function gzip-compresses and base64-encodes the data. This significantly reduces the size of the serialized prop passed through the component boundary.
...
Prefetches specific model data from a server endpoint. Useful when you only need a subset of models for a particular page.
...
`PrefetchResult` is a plain object and can be passed directly through the Server Component boundary via `JSON.stringify` / props without additional serialization.

## Complete SSR bootstrap example
...
export default async function RootLayout({
  ...
  let bootstrapData: string | undefined;
  ...
  try {
    ...
    bootstrapData = await encodeBootstrapSnapshot(snapshot);
  } catch {
  ...
  return (
    <html>
      <body>
        <AppProviders bootstrapData={bootstrapData}>{children}</AppProviders>
      </body>
    </html>
  );
}
