...
See [Server utilities](../packages/next/server) for all `prefetchBootstrap` options.
...
See [Client utilities](../packages/next/client) for the full `seedStorageFromBootstrap` API.
