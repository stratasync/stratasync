...
The React hooks in `@stratasync/react` are thin wrappers around `SyncClient`. See [React](../react) for details.
...
See [Undo and redo](undo-redo) for details.
...
See [Event system](events) for the full reference.
