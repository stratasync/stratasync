...
- Logger is injected via `SyncLogger` interface — no pino dependency; noop logger used when none provided
- Redis is optional — falls back to in-memory delta bus for single-server / dev mode
