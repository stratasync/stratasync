...
- `computedCollection` requires a synchronous `getAll(modelName)` on the attached store. `createSyncClient()` provides this from the identity map.
...
- Use `createDirtyTracker()` when you need reactive dirty state. Do not manually track changes.
