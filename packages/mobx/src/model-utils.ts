 ...
 */
export const toPlainObject = <T extends Model>(
  model: T
 ...
 */
export const cloneModelData = <T extends Model>(
  model: T
): Record<string, unknown> => ({ ...toPlainObject(model) });

/**
 * Compares two model instances' data and returns changed fields.
 */
export const diffModels = <T extends Model>(
  a: T,
  b: T
 ...
 */
export const isModelDirty = (model: Model): boolean => {
  const tracker = (model as unknown as Record<symbol, unknown>)[
    DIRTY_TRACKER_SYMBOL
