...
tests/
  ...
  sync-engine.test.ts                — full sync lifecycle: bootstrap → steady-state (14 tests)
```
...
- `"sent"` transactions reset to `"queued"` on reconnect — prevents lost mutations but can cause duplicate sends if server already processed them
...
- `createSyncClient(options)` is the only entry point — returns an immutable `SyncClient` object with bound methods (closure pattern, no class)
...
- Queries execute against in-memory identity maps — `where` predicates, `orderBy` sort functions, `limit`/`offset` pagination
...
- Rebase strategy defaults to `"server-wins"` — conflicting local mutations are rolled back. `"client-wins"` and `"merge"` update the original baseline instead
